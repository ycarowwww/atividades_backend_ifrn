...
def list_usuario_view(request, id=None):
    ...
    return render(
        request, template_name="usuario/usuario.html", context=context, status=200
    )
...
def edit_usuario_view(request):
    ...
    if request.method == "POST":
        ...
        verifyEmail = (
            Usuario.objects.filter(user__email=request.POST["email"])
    ...
    return render(
        request, template_name="usuario/usuario-edit.html", context=context, status=200
    )
